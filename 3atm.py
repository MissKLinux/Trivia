...
# Initialize variables to keep track of trivia stats
...
async def trivia(ctx):
    ...
    if ctx.author.id in user_scores and (time.time() - user_scores[ctx.author.id]["last_trivia_time"]) < cooldown:
        await ctx.send(f"{ctx.author.mention}, you are on cooldown. Please wait before starting a new trivia round.")
        return

    question = random.choice(trivia_questions)
    await ctx.send(f'**Trivia Question:**\n{question["question"]}')

    user_scores[ctx.author.id] = {"score": 0, "remaining_tries": max_tries, "last_trivia_time": time.time()}
    answer_flag = False

    while user_scores[ctx.author.id]["remaining_tries"] > 0 and not answer_flag:
        start_time = time.time()
        try:
            response = await bot.wait_for(
                'message',
                timeout=30,  # Set a time limit for answering (in seconds)
                check=lambda message: message.author == ctx.author
            )
            elapsed_time = time.time() - start_time

            total_questions_asked += 1
            user_scores[ctx.author.id]["last_trivia_time"] = time.time()

            if response.content.lower() == question['answer'].lower():
                total_correct_answers += 1
                user_scores[ctx.author.id]["score"] += 1
                result = "Correct"
            else:
                result = "Incorrect"

            await ctx.send(f'{ctx.author.mention}, your answer is {result}.')
            if result == "Incorrect":
                await ctx.send(
                    f'{ctx.author.mention}, you have {user_scores[ctx.author.id]["remaining_tries"] - 1} guesses left.')

            print(
                f'| {get_timestamp():<19} | {ctx.author.name:<25} | {question["question"][:40]:<40} | {response.content[:20]:<20} | {result:<8} | {elapsed_time:<10.2f} |')
            print(
                f'| {"":<19} | {"":<25} | {"Correct Answer:":<40} | {question["answer"][:20]:<20} | {"":<8} | {"":<10} |')

            if result == "Correct":
                answer_flag = True
            else:
                user_scores[ctx.author.id]["remaining_tries"] -= 1

        except asyncio.TimeoutError:
            user_scores[ctx.author.id]["remaining_tries"] -= 1
            await ctx.send(
                f'{ctx.author.mention}, you ran out of time. You have {user_scores[ctx.author.id]["remaining_tries"]} guesses left.')
            print(
                f'| {get_timestamp():<19} | {ctx.author.name:<25} | {question["question"][:40]:<40} | {"":<20} | Timeout | {elapsed_time:<10.2f} |')
            print(
                f'| {"":<19} | {"":<25} | {"Correct Answer:":<40} | {question["answer"][:20]:<20} | {"":<8} | {"":<10} |')

    if not answer_flag:
        await ctx.send(
            f'{ctx.author.mention}, you have no more guesses left. The correct answer was: {question["answer"]}')
